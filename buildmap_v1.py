...
def build_map(level,map,affix1,affix2,affix3,affix4):
    l_map = Image.new('RGBA', (width*5,height), (255,255,255,255))
    l_map.paste(create_level(dungeons[map],level), (width*0,height*0))
    l_map.paste(affixes[affix1], (width*1, height*0))
    l_map.paste(affixes[affix2], (width*2, height*0))
    l_map.paste(affixes[affix3], (width*3, height*0))
    l_map.paste(affixes[affix4], (width*4, height*0))
    return(l_map)

def build_match(map1,map2,map3):
    l_match = Image.new('RGBA', (width*5,height*3), (255,255,255,255))
    l_match.paste(map1,(width*0, height*0))
    l_match.paste(map2,(width*0, height*1))
    l_match.paste(map3,(width*0, height*2))
    return(l_match)
...
matchlist = pd.read_csv('Spring_1.csv',delimiter=',')

matches = len(matchlist.count())-1
match_graphics = []
for i in range(0,matches-1):
    x = []
    for j in range(0,3):
        x.append(build_map(keylevel,matchlist.ix[i,(j*5)+1],matchlist.ix[i,(j*5)+2],matchlist.ix[i,(j*5)+3],matchlist.ix[i,(j*5)+4],matchlist.ix[i,(j*5)+5]))
    y = build_match(x[0],x[1],x[2])
    match_graphics.append(y)

# matchtest.show()

# match_graphics[0].show()
